...
#[cfg(test)]
mod tests {
    use super::compute_primitives;
    use crate::cache::{mmap_expr_bin, write_expr_bin};
    use crate::compute::{SIMD_EPS, scalar};
    use crate::core::types::GeneSet;
    use crate::data::ExpressionSoA;
    use crate::input::{GeneIndex, resolve_all_genesets};
    ...
    fn make_fixture(samples: usize) -> (ExpressionSoA, GeneSet, Vec<String>) {
        let genes = vec![
            "MT-ND1", "MT-CYB", "MT-CO1", "MT-ATP6", "MT-ATP8", "NDUFS1", "SDHA", "UQCRC1",
            "COX4I1", "ATP5F1A", "SOD2", "PINK1", "MFN1", "DNM1L", "TFAM",
        ];
    ...
    fn compute_primitives_matches_scalar() {
        ...
        let offsets = crate::compute::GeneOffsets::from_resolved(&resolved);
